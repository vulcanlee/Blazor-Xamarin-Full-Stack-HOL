...
namespace AA24
{
    ...
    class Program
    {
        ...
        {
            (APIResult apiResult, LoginResponseDto loginResponse) = await LoginPostAsync(new LoginRequestDto()
            {
                Account = "user1",
                Password = "123"
            });
            Console.WriteLine($"存取權杖 : {loginResponse.Token}");
            Console.WriteLine($"更新權杖 : {loginResponse.RefreshToken}");
            Console.WriteLine($"準備更新存取權杖");
            (apiResult, loginResponse) = await RefreshTokenAsync(loginResponse.RefreshToken);
            Console.WriteLine($"存取權杖 : {loginResponse.Token}");
            Console.WriteLine($"更新權杖 : {loginResponse.RefreshToken}");
        }
        ...
        private static async Task<(APIResult, LoginResponseDto)> RefreshTokenAsync(string refreshToken)
        {
            APIResult apiResult;
            LoginResponseDto loginResponseDto = null;
            using (HttpClient client = new HttpClient())
            {
                ...
                {
                    #region 準備開啟連線
                    string endPoint = $"https://localhost:5001/api/login/RefreshToken";
                    HttpResponseMessage response = null;
