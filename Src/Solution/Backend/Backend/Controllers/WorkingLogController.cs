...
namespace Backend.Controllers
{
    ...
    [ApiController]
    public class WorkingLogController : ControllerBase
    {
        private readonly IWorkingLogService WorkingLogService;
        private readonly IMapper mapper;

        #region 建構式
        public WorkingLogController(IWorkingLogService WorkingLogService,
            IMapper mapper)
        {
        ...
        public async Task<IActionResult> Get()
        {
            ...
            var records = await WorkingLogService.GetAsync(dataRequest);
            var result = mapper.Map<List<WorkingLogDto>>(records.Result);
        ...
        public async Task<IActionResult> Delete([FromRoute] int id)
        {
            ...
            if (record != null)
            {
                ...
                VerifyRecordResult verify = await WorkingLogService.BeforeDeleteCheckAsync(record);
                if (verify.Success == false)
                {
                    apiResult = APIResultFactory.Build(false, StatusCodes.Status200OK,
                          ErrorMessageMappingHelper.Instance.GetErrorMessage(verify.MessageId),
                ...
                var verifyRecordResult = await WorkingLogService.DeleteAsync(id);
                if (verifyRecordResult.Success)
                {
                    apiResult = APIResultFactory.Build(true, StatusCodes.Status202Accepted,
                        ErrorMessageEnum.None, payload: null);
