...
namespace Backend.Controllers
{
    ...
    public class ProjectController : ControllerBase
    {
        ...
        public async Task<IActionResult> Get()
        {
            ...
            var records = await ProjectService.GetAsync(dataRequest);
            var result = mapper.Map<List<ProjectDto>>(records.Result);
        ...
        {
            APIResult apiResult;
            var record = await ProjectService.GetAsync(id);
            var result = mapper.Map<ProjectDto>(record);
            if (record != null)
            {
                apiResult = APIResultFactory.Build(true, StatusCodes.Status200OK,
        ...
        #region U 更新
        [HttpPut("{id}")]
        public async Task<IActionResult> Put([FromRoute] int id, [FromBody] ProjectDto data)
        {
            APIResult apiResult;
            ...
            if (record != null)
            {
                ProjectAdapterModel recordTarget = mapper.Map<ProjectAdapterModel>(data);
                recordTarget.Id = id;
                var result = mapper.Map<ProjectDto>(recordTarget);

                #region 修改記錄前的紀錄完整性檢查
                VerifyRecordResult verify = await ProjectService.BeforeUpdateCheckAsync(record);
                if (verify.Success == false)
                {
                    apiResult = APIResultFactory.Build(false, StatusCodes.Status200OK,
                          ErrorMessageMappingHelper.Instance.GetErrorMessage(verify.MessageId),
                ...
                var verifyRecordResult = await ProjectService.UpdateAsync(recordTarget);
                if (verifyRecordResult.Success)
                {
        ...
        public async Task<IActionResult> Delete([FromRoute] int id)
        {
            ...
            if (record != null)
            {
                ...
                VerifyRecordResult verify = await ProjectService.BeforeDeleteCheckAsync(record);
                if (verify.Success == false)
                {
                    apiResult = APIResultFactory.Build(false, StatusCodes.Status200OK,
                          ErrorMessageMappingHelper.Instance.GetErrorMessage(verify.MessageId),
                ...
                var verifyRecordResult = await ProjectService.DeleteAsync(id);
                if (verifyRecordResult.Success)
                {
                    apiResult = APIResultFactory.Build(true, StatusCodes.Status202Accepted,
                        ErrorMessageEnum.None, payload: null);
