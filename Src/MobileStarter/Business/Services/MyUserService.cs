...
{
    public class MyUserService : BaseWebAPI<MyUserDto>
    {
        private readonly AppStatus appStatus;

        public MyUserService(AppStatus appStatus)
            : base()
        {
            this.Url = "/api/MyUser";
        ...
        public async Task<APIResult> PostAsync(MyUserDto item)
        {
            #region 指定此次呼叫 Web API 要執行參數
            Token = appStatus.SystemStatus.Token;
        ...
        public async Task<APIResult> PutAsync(MyUserDto item)
        {
            #region 指定此次呼叫 Web API 要執行參數
            Token = appStatus.SystemStatus.Token;
            ApiResultIsCollection = false;
        ...
        public async Task<APIResult> DeleteAsync(MyUserDto item)
        {
            #region 指定此次呼叫 Web API 要執行參數
            Token = appStatus.SystemStatus.Token;
            ApiResultIsCollection = false;
